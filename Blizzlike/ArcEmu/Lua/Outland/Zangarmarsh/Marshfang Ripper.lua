...
function hittest_1(pUnit, Event)
	if(pUnit:GetHealthPct() < 90) then
		pUnit:RemoveEvents();
		pUnit:FullCastSpellOnTarget(17008,p Unit:GetClosestPlayer())
...
function hittest_4(pUnit, Event)
	if(pUnit:GetHealthPct() < 28) then
		pUnit:RemoveEvents();
		pUnit:FullCastSpellOnTarget(35333,p Unit:GetClosestPlayer())
...
function hittest_5(pUnit, Event)
	if(pUnit:GetHealthPct() < 14) then
		pUnit:RemoveEvents();
		pUnit:FullCastSpellOnTarget(35334,p Unit:GetClosestPlayer())
	end
end
