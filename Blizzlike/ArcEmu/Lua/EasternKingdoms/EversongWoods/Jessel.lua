...
local EMOTE = ChatField.CHAT_MSG_TEXT_EMOTE
...
function Jessel_Text(Unit,Event)
	...
	Unit:SendChatMessage(EMOTE, UNIVERSAL, "Jessel wakes up, startled.")
	Unit:RegisterEvent("Jessel_Talk", 2000, 1)
end

function Jessel_Talk(Unit,Event)
	Unit:SendChatMessage(SAY, THELASSIAN, "Was... Was it all a dream? Azuremyst, Bloodmyst, Zangarmarsh, Shadowmoon... even Zul'Aman... Jane! Jane where are you!")
...
local Jane = Unit:GetCreatureNearestCoords(X,Y,Z,20098)
	Jane:SendChatMessage(SAY, THELASSIAN, "I'm right here, daddy. You've been asleep all day long. You sure must be tired!")
	Jane:SendChatMessage(EMOTE, UNIVERSAL, "Jane resumes playing on the beach.")
	Unit:RegisterEvent("Jessel_TalkBack", 3000, 1)
end

function Jessel_TalkBack(Unit,Event)
	Unit:CastSpell(35112)
	Unit:SendChatMessage(SAY, THELASSIAN, "Ah, wonderful. It was all just a dream...")
end
